...
fmtcheck:
	...
	@dirty=$$(find . -type f -name '*.go' ! -path './refs/*' ! -path './.gocache/*' ! -path './.gomodcache/*' -exec gofmt -l {} + 2>/dev/null); \
	if [ -n "$$dirty" ]; then \
...
imports:
	...
	find . -type f -name '*.go' ! -path './refs/*' ! -path './.gocache/*' ! -path './.gomodcache/*' -exec goimports -w -local github.com/komsit37/yf-go {} +

importscheck:
	@if command -v goimports >/dev/null 2>&1; then \
		out=$$(find . -type f -name '*.go' ! -path './refs/*' ! -path './.gocache/*' ! -path './.gomodcache/*' -exec goimports -l {} + 2>/dev/null); \
		if [ -n "$$out" ]; then \
...
build:
	mkdir -p .gocache
	mkdir -p .gomodcache
	GOCACHE=$(CURDIR)/.gocache GOMODCACHE=$(CURDIR)/.gomodcache go build -o yf ./cmd/yf

run:
	mkdir -p .gocache
	mkdir -p .gomodcache
	GOCACHE=$(CURDIR)/.gocache GOMODCACHE=$(CURDIR)/.gomodcache go run ./cmd/yf --help
